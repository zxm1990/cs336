...
from cs336_basics.attention import MultiSelfAttention
from cs336_basics.transformer import TransformerBlock, TransformerLM
from cs336_basics.cross_entropy import cross_entropy
from cs336_basics.adamw import MyAdamw, get_lr_cosine_schedule, gradient_clipping
...
def run_multihead_self_attention(
    ...
    """
    attention = MultiSelfAttention(d_model, num_heads, device=in_features.device, dtype=in_features.dtype)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    attention = MultiSelfAttention(d_model, num_heads, theta, max_seq_len, in_features.device, in_features.dtype)
    attention.load_state_dict(
        {
            "q_proj.weight": q_proj_weight,
            "k_proj.weight": k_proj_weight,
...
def run_transformer_lm(
    ...
    """
    transformerlm = TransformerLM(vocab_size, d_model, num_heads, num_layers, d_ff, rope_theta, context_length,
    device = in_indices.device, dtype = torch.float32)
    # rope 的cos 需要单独处理
    transformerlm.load_state_dict(weights, strict=False)
    out =  transformerlm(in_indices)
    return out
...
def get_tokenizer(
    ...
    """
    return BPETokenizer.new_instance(vocab, merges, special_tokens)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    _tokenizer = BPETokenizer(vocab_size, num_chunk=8, special_tokens=special_tokens, **kwargs)
